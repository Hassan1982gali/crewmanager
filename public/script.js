...
function confirmDelete(msg = 'تأكيد الحذف') {
  const pass = prompt(`🔒 ${msg}\n\nأدخل كلمة المرور للمتابعة:`);
  if (pass === null) return false; // ضغط Cancel
  if (pass !== '1982') {
...
// ✅ تحديد الحسابات والصلاحيات (ملاحظة: الأدوار تُجلب من Supabase)
const ADMIN_EMAILS = ['admin@marine.com'];
const VIEWER_EMAILS = ['viewer@marine.com'];
...
function disableEditing() {
  ...
  // ✅ منع حفظ التعديلات
  document
    .querySelectorAll("button[onclick^='saveEditCrewMember']")
    .forEach((button) => {
  ...
  document
    .querySelectorAll('.filter-search, .checkbox-list input')
    .forEach((input) => {
...
function getServiceDuration(entry, nextEntry, prevEntry) {
  ...
  if (
    ...
    join &&
    !leave &&
    nextEntry?.ship === 'استراحة' &&
    nextEntry?.leave_date
  ) {
...
async function loadEmployees(callback = null) {
  ...
  const { data: employees, error } = await sb
    .from('crew_list')
    .select(
      'id, name, job_number, rank, ship, join_date, join_duration, leave_date, leave_duration, status, note, is_complete'
    );
  ...
  if (!employees || employees.length === 0) {
    console.warn('⚠️ قاعدة البيانات فارغة أو الجلسة منتهية');
    return;
  }
...
async function showSeaTime(employeeId) {
  ...
  if (error) {
    ...
    tableBody.innerHTML =
      "<tr><td colspan='7'>حدث خطأ أثناء تحميل البيانات</td></tr>";
  ...
  if (!data || data.length === 0) {
    tableBody.innerHTML =
      "<tr><td colspan='7'>لا يوجد سجل لهذا الموظف</td></tr>";
  } else {
    // ✅ ترتيب حسب التاريخ
    data.sort((a, b) => {
    ...
    reversedData.forEach((entry) => {
      ...
      row.innerHTML = `
        ...
        <td><button onclick="deleteHistoryRecord('${
          entry.id
...
async function deleteHistoryRecord(historyId) {
  if (!confirmDelete('هل تريد حذف هذا السجل؟')) return;
...
function getSelectedFilters(filterId) {
  ...
  return filter
    ? Array.from(filter.selectedOptions).map((option) => option.value)
    : [];
}
...
function displayEmployees(employees, historyMap = new Map()) {
  ...
  const showIncompleteOnly = document.getElementById(
  ...
  employees.forEach((crew, index) => {
    ...
    row.innerHTML = `
      <td>${index + 1}</td>
        <td style="${
          crew.is_complete === false
            ? 'background-color:#ffe6e6; color:#d32f2f; font-weight:bold;'
            : ''
        }">
  ${
    crew.is_complete === false
      ? '<span title="ملف غير مكتمل (نقص شهادات)">⚠️</span> '
      : ''
  }${crew.name ?? 'غير معروف'}
</td>
<td>
        <input type="text" class="job-number-field no-print" data-id="${
          crew.id
        }" value="${
      crew.job_number ?? ''
    }" placeholder="—" style="width: 70px; text-align: center; border: 1px dashed transparent; background: transparent; font-family: 'Cairo', sans-serif;">
        <div class="print-job-number-only" style="display: none;">${
          crew.job_number ?? '—'
        }</div>
      </td>
  ...
  <div class="history-wrap">
    ${historyHTML}
      ...
      <td class="actions-cell">
        <button class="action-btn btn-edit" onclick="editCrewMember('${
          crew.id
        }')">✏ تعديل</button>
        <button class="action-btn btn-delete" onclick="deleteCrewMember('${
          crew.id
        }')">🗑 حذف</button>
        <button class="action-btn btn-history" onclick="showSeaTime('${
          crew.id
        }')">📄 السجل</button>
        <button class="action-btn btn-profile" onclick="showEmployeeProfile('${
          crew.id
        }')">📋 ملف الموظف</button>
      </td>
    `;
  ...
  setTimeout(() => {
    document.querySelectorAll('.note-field').forEach((noteField) => {
      ...
      noteField.addEventListener('blur', async (event) => {
        ...
        if (error) {
          ...
          const printNoteDiv =
            noteField.parentElement.querySelector('.print-note-only');
          if (printNoteDiv) {
    ...
    document.querySelectorAll('.job-number-field').forEach((jobField) => {
      ...
      jobField.addEventListener('blur', async (event) => {
        ...
        const { error } = await sb
          .from('crew_list')
          .update({ job_number: newJobNum })
          .eq('id', crewId);

        if (!error) {
          console.log('✅ تم حفظ الرقم الوظيفي!');
          // تحديث نسخة الطباعة
          const printDiv = jobField.parentElement.querySelector(
            '.print-job-number-only'
          );
...
function sortByDuration(type) {
  ...
  rows.sort((a, b) => {
    // ✅ العمود 6 = مدة الالتحاق، العمود 8 = مدة النزول (بعد إضافة الرقم الوظيفي)
    let durationA =
      parseInt(
        a.cells[type === 'join_duration' ? 6 : 8].textContent
          .replace(' يوم', '')
          .trim()
      ) || 0;
    let durationB =
      parseInt(
        b.cells[type === 'join_duration' ? 6 : 8].textContent
          .replace(' يوم', '')
          .trim()
      ) || 0;
...
function ensureFiltersContainer() {
  ...
  if (!filtersContainer) {
    console.warn(
      '⚠ لم يتم العثور على .filters-container، سيتم إنشاؤها تلقائيًا.'
    );
...
async function loadFilterOptions(column, containerId) {
  ...
  const uniqueValues = [
    ...new Set(data.map((row) => row[column]).filter(Boolean)),
  ];
...
function filterCrew() {
  ...
  let selectedRanks = Array.from(
    document.querySelectorAll('#ranks-container input[type="checkbox"]:checked')
  ).map((cb) => cb.value);
  let selectedShips = Array.from(
    document.querySelectorAll('#ships-container input[type="checkbox"]:checked')
  ).map((cb) => cb.value);
  let selectedStatuses = Array.from(
    document.querySelectorAll(
      '#status-container input[type="checkbox"]:checked'
    )
  ).map((cb) => cb.value);

  console.log('✅ الفلاتر المحددة:', {
    selectedRanks,
    selectedShips,
    selectedStatuses,
  });
  ...
  rows.forEach((row) => {
    ...
    let matchesRank =
      selectedRanks.length === 0 || selectedRanks.includes(rankColumn);
    let matchesShip =
      selectedShips.length === 0 || selectedShips.includes(shipColumn);
    let matchesStatus =
      selectedStatuses.length === 0 || selectedStatuses.includes(statusColumn);

    // ✅ إظهار الصف إذا تطابق مع الفلاتر، وإخفاؤه إذا لم يتطابق
    row.style.display =
      matchesRank && matchesShip && matchesStatus ? '' : 'none';
  });
    ...
    .then(async ({ error }) => {
      if (error) {
        ...
        if (changed) {
          ...
          if (joinDate && leaveDate) {
            historyRecord.duration =
              parseInt(calculateDuration(joinDate, leaveDate)) || 0;
          } else if (joinDate && !leaveDate) {
            historyRecord.duration =
              parseInt(calculateDuration(joinDate, new Date())) || 0;
          } else if (!joinDate && leaveDate) {
            historyRecord.duration =
              parseInt(calculateDuration(leaveDate, new Date())) || 0;
          } else {
...
async function loadDropdown(selectId, column, selectedValue) {
  ...
  if (column === 'rank') {
    const { data } = await sb
      ...
      .select('value')
      .eq('type', 'rank');
    const fromDB = (data || []).map((r) => r.value);
    values = [
      ...rankOrder.filter((r) => fromDB.includes(r)),
      ...fromDB.filter((r) => !rankOrder.includes(r)),
    ];
  } else {
    const typeMap = { ship: 'ship', status: 'status' };
    const { data } = await sb
      .from('app_options')
      .select('value')
      .eq('type', typeMap[column] || column);
...
async function loadAddModalData() {
  try {
    const { data, error } = await sb.from('app_options').select('type, value');
    if (error || !data) {
      console.error('❌ خطأ loadAddModalData:', error);
      return;
    }

    const fromDB_ranks = data
      .filter((r) => r.type === 'rank')
      .map((r) => r.value);
    const ships = data.filter((r) => r.type === 'ship').map((r) => r.value);
    const statuses = data
      .filter((r) => r.type === 'status')
      .map((r) => r.value);

    // الرتب مرتبة حسب rankOrder
    const ranks = [
      ...rankOrder.filter((r) => fromDB_ranks.includes(r)),
      ...fromDB_ranks.filter((r) => !rankOrder.includes(r)),
    ];
...
async function deleteCrewMember(memberId) {
  if (!confirmDelete('هل تريد حذف هذا الموظف نهائياً؟')) return;
...
async function loadHistory() {
  const { data, error } = await sb.from('history').select('*'); // نحتاج كل الأعمدة لحساب المدة
...
// (دمج مع DOMContentLoaded الرئيسي أعلاه)
...
async function loadFilterOptions(column, containerId) {
  ...
  if (column === 'rank') {
    // ✅ جلب الرتب من app_options مرتبة حسب rankOrder
    const { data, error } = await sb
      .from('app_options')
      .select('value')
      .eq('type', 'rank');
    ...
    const fromDB = data.map((r) => r.value);
    // نرتبها حسب rankOrder، والجديدة اللي مو بالقائمة تجي بالآخر
    uniqueValues = [
      ...rankOrder.filter((r) => fromDB.includes(r)),
      ...fromDB.filter((r) => !rankOrder.includes(r)),
    ];
    ...
    const { data, error } = await sb
      .from('app_options')
      .select('value')
      .eq('type', typeMap[column]);
    if (error) {
      console.error(`❌ خطأ:`, error);
      return;
    }
    uniqueValues = data.map((r) => r.value);
  }
...
function generatePrintSummary() {
  ...
  rows.forEach((row) => {
    if (row.style.display !== 'none') {
      // ✅ يأخذ فقط البيانات الظاهرة (المفلترة)
      let rank = row.cells[3].textContent.trim(); // ✅ العمود 3 هو الرتبة
      if (!summaryData[rank]) {
        summaryData[rank] = 0;
      }
      summaryData[rank]++;
  ...
  if (Object.keys(summaryData).length === 0) {
    summaryPrintBody.innerHTML =
      "<tr><td colspan='2'>لا توجد بيانات مطابقة للفلاتر</td></tr>";
    return;
  }
...
function printFilteredData() {
  ...
  let filteredRows = Array.from(
    document.querySelectorAll('#employee-table-body tr')
  ...
  printWindow.document.write(`
        ...
        <body>
            ...
            <div class="meta-info">
                <div><strong>الفلاتر المطبقة:</strong><br>${
                  generateFilterSummaryHTML() || 'الكل (بدون فلاتر)'
                }</div>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>الحالة</th>
                        ${
                          document.body.classList.contains('print-history')
                            ? `<th>الخدمة السابقة</th>`
                            : ''
                        }
                        <th>الملاحظات</th>
                    </tr>
                ...
                <tbody>
                  ${filteredRows
                    .map((row) => {
                      ...
                      const showHistory =
                        document.body.classList.contains('print-history');
                      let rowHtml = Array.from(cells)
                        .map((cell, index) => {
                          if (!showHistory && index === 10) return ''; // الخدمة السابقة (صار رقمها 10)
                          if (index === 12) return ''; // الإجراءات (صار رقمها 12)

                          if (index === 2) {
                            // الرقم الوظيفي للطباعة
                            const printJobNum = cell.querySelector(
                              '.print-job-number-only'
                            );
                            return `<td>${
                              printJobNum ? printJobNum.innerText : ''
                            }</td>`;
                          }
                          if (index === 11) {
                            // الملاحظات للطباعة
                            const printNote =
                              cell.querySelector('.print-note-only');
                            return `<td>${
                              printNote ? printNote.innerHTML : ''
                            }</td>`;
                          }
...
function printSeaTime() {
  ...
  const name =
    document
      .getElementById('seaTimeModal')
      ?.getAttribute('data-employee-name') || 'بدون اسم';
...
function updateSummaryTable() {
  ...
  rows.forEach((row) => {
    let rank = row.cells[3].textContent.trim(); // ✅ العمود 3 هو الرتبة (بعد إضافة الرقم الوظيفي)
...
function getFilteredEmployees() {
  ...
  rows.forEach((row) => {
    if (row.style.display !== 'none') {
      let crew = {
        rank: row.cells[3].textContent.trim(), // ✅ العمود 3 هو الرتبة
      };
      employees.push(crew);
    }
  });
...
function exportFilteredToExcel() {
  ...
  rows.forEach((row) => {
    ...
    // الترتيب: 0=# | 1=الاسم | 2=الرقم الوظيفي | 3=الرتبة | 4=الناقلة | 5=تاريخ الالتحاق | 6=مدة الالتحاق | 7=تاريخ النزول | 8=مدة النزول | 9=الحالة | 10=الخدمة السابقة | 11=الملاحظات
    const printJobNum = cells[2]?.querySelector('.print-job-number-only');
    const printNote = cells[11]?.querySelector('.print-note-only');
    const rowData = [
      cells[1]?.textContent.trim(), // الاسم
      printJobNum ? printJobNum.innerText.trim() : '', // الرقم الوظيفي
      cells[3]?.textContent.trim(), // الرتبة
      cells[4]?.textContent.trim(), // الناقلة
      cells[5]?.textContent.trim(), // تاريخ الالتحاق
      cells[6]?.textContent.trim(), // مدة الالتحاق
      cells[7]?.textContent.trim(), // تاريخ النزول
      cells[8]?.textContent.trim(), // مدة النزول
      cells[9]?.textContent.trim(), // الحالة
      printNote ? printNote.innerText.trim() : '', // الملاحظات
    ];
    data.push(rowData);
  });
...
function sortByLeaveDuration(order) {
  ...
  rows.sort((a, b) => {
    let durationA = a.querySelector('.leave-duration')
      ? parseInt(
          a
            .querySelector('.leave-duration')
            .textContent.replace(' يوم', '')
            .trim()
        )
      : 0;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ✅ فلتر نقص الشهادات
  document
    .getElementById('filter-incomplete-only')
  ...
  const restFilter = document.getElementById('toggle-rest-filter');
  if (restFilter) {
    restFilter.addEventListener('change', () => {
      loadEmployees();
    });
  }
});
...
// ==========================================
// ✅ إدارة القوائم (app_options)
// ==========================================

let currentOptionsTab = 'rank';
...
function openOptionsManager() {
  document.getElementById('optionsManagerModal').style.display = 'flex';
  switchTab('rank');
}

function closeOptionsManager() {
  document.getElementById('optionsManagerModal').style.display = 'none';
}

function switchTab(type) {
  currentOptionsTab = type;
  ['rank', 'ship', 'status'].forEach((t) => {
    const btn = document.getElementById(`tab-${t}`);
    btn.classList.toggle('opt-tab-active', t === type);
  });
  document.getElementById(
    'new-option-input'
  ).placeholder = `أدخل ${optionLabels[type]} جديدة...`;
  document.getElementById('new-option-input').value = '';
  document.getElementById('options-msg').textContent = '';
  renderOptionsList(type);
}

async function renderOptionsList(type) {
  ...
  const { data, error } = await sb
    ...
    .order('value');
  if (error) {
    container.innerHTML = `<div style="color:red;">❌ فشل التحميل</div>`;
    return;
  }

  if (!data || data.length === 0) {
    container.innerHTML = `<div style="text-align:center; padding:20px; color:#888;">لا توجد عناصر</div>`;
    return;
  }

  // ترتيب الرتب حسب rankOrder
  let sorted = data;
  if (type === 'rank') {
    sorted = [
      ...rankOrder
        .filter((r) => data.find((d) => d.value === r))
        .map((r) => data.find((d) => d.value === r)),
      ...data.filter((d) => !rankOrder.includes(d.value)),
    ];
  }
  ...
  container.innerHTML = sorted
    .map(
      (item) => `
    <div class="opt-item">
      <span class="opt-item-name">${item.value}</span>
      <button class="opt-del-btn" onclick="deleteOption('${
        item.id
      }', '${item.value.replace(/'/g, "\\'")}')">🗑 حذف</button>
    </div>
  `
    )
    .join('');
...
async function addOption() {
  ...
  if (!value) {
    msg.style.color = 'red';
    msg.textContent = '⚠️ أدخل قيمة أولاً';
    return;
  }

  const { error } = await sb
    .from('app_options')
    .insert([{ type: currentOptionsTab, value }]);

  if (error) {
    if (error.code === '23505') {
      msg.style.color = 'orange';
      msg.textContent = `⚠️ "${value}" موجودة مسبقاً`;
    } else {
      msg.style.color = 'red';
      msg.textContent = '❌ فشل الإضافة: ' + error.message;
    }
    return;
  }

  msg.style.color = 'green';
  msg.textContent = `✅ تم إضافة "${value}"`;
  input.value = '';

  // تحديث القائمة والفلاتر
  renderOptionsList(currentOptionsTab);
  refreshAllDropdowns();
}

async function deleteOption(id, value) {
  const msg = document.getElementById('options-msg');
  if (!confirmDelete(`حذف "${value}" من القوائم؟`)) return;

  const { error } = await sb.from('app_options').delete().eq('id', id);

  if (error) {
    msg.style.color = 'red';
    msg.textContent = '❌ فشل الحذف: ' + error.message;
    return;
  }

  msg.style.color = 'green';
  msg.textContent = `✅ تم حذف "${value}"`;
...
// إعادة تحميل كل الفلاتر والـ dropdowns بعد أي تغيير
...
window.closeOptionsManager = closeOptionsManager;
